...
// L/R  -> 3.3 V = RIGHT
//
// Componente configurado em STEREO.
// O microfone esta fisicamente no canal RIGHT.
// ============================================================
...
#define AUDIO_WS_GPIO GPIO_NUM_40
#define AUDIO_DATA_GPIO GPIO_NUM_41
...
static float voltage_to_current_a(float voltage_mv) {
  ...
  float measured_current = fabsf(delta_mv) / ACS712_SENSITIVITY_ADC_MV_PER_A;

  if (measured_current < CURRENT_DEADBAND_A) {
...
static void update_motor_state(float measured_current) {
  ...
  case MOTOR_STATE_OFF: {
    if (measured_current >= MOTOR_ON_THRESHOLD_A) {
      ...
      ESP_LOGI(TAG_ACS, "Motor -> STARTUP");
    }
...
static void init_audio(void) {
  ...
  inmp441_config_t audio_config = {};

  audio_config.bclk_gpio = AUDIO_BCLK_GPIO;

  audio_config.ws_gpio = AUDIO_WS_GPIO;

  audio_config.data_gpio = AUDIO_DATA_GPIO;

  audio_config.sample_rate = AUDIO_SAMPLE_RATE;

  esp_err_t err = inmp441_init(&audio_config);

  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Falha inicializando INMP441: %s",
             esp_err_to_name(err));

    ESP_ERROR_CHECK(err);
  }

  ESP_LOGI(TAG_AUDIO, "INMP441 inicializado");
}

// ============================================================
// INMP441 - RMS CANAL RIGHT
//
// O driver captura:
// LEFT, RIGHT, LEFT, RIGHT...
//
// Como L/R esta ligado a 3.3 V,
// usamos somente o canal RIGHT.
//
// O componente trabalha com palavras de 32 bits,
// contendo audio de 24 bits.
// ============================================================

static float read_audio_rms(void) {
  size_t samples_read = 0;

  esp_err_t err =
      inmp441_read_samples(audio_buffer, AUDIO_BUFFER_SIZE, &samples_read);

  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Erro lendo INMP441: %s", esp_err_to_name(err));

    return 0.0f;
  }

  if (samples_read < 2) {
    ESP_LOGW(TAG_AUDIO, "Nenhuma amostra de audio");
  ...
  for (size_t i = 1; i < samples_read; i += 2) {
    // O INMP441 fornece 24 bits validos
    // dentro da palavra I2S de 32 bits.

    int32_t sample = audio_buffer[i] >> 8;

    double normalized = static_cast<double>(sample) / 8388608.0;
  ...
  return static_cast<float>(sqrt(sum_sq / static_cast<double>(right_samples)));
}
...
extern "C" void app_main(void) {
  ...
  // ACS712
  //
  // MOTOR DESLIGADO DURANTE A CALIBRACAO.
  // ========================================================
  ...
  while (true) {
    ...
    printf("P101 | I=%.3f A | AUDIO_RMS=%.5f | STATE=%s | PEAK=%.3f A\n",
           current_a, audio_rms, motor_state_to_string(motor_state),
           startup_peak_current_a);
