...
#define BNO085_SENSOR_ID_MAX  (0x2A)
                               ...
                               uint32_t report_interval_us)
{
    ...
    if (sensor_id == 0 || sensor_id > BNO085_SENSOR_ID_MAX) {
...
{
    if (out_roll) {
        *out_roll = atan2f(2.0f * (real * i + j * k), 1.0f - 2.0f * (i * i + j * j));
    }

    if (out_pitch) {
        float sin_pitch = 2.0f * (real * j - k * i);
        sin_pitch = (sin_pitch > 1.0f) ? 1.0f : (sin_pitch < -1.0f) ? -1.0f : sin_pitch;
        *out_pitch = asinf(sin_pitch);
    }

    if (out_yaw) {
        *out_yaw = atan2f(2.0f * (real * k + i * j), 1.0f - 2.0f * (j * j + k * k));
    }
}
