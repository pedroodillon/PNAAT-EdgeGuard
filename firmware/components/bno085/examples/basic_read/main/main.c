...
void app_main(void)
{
    /* Install the console UART driver with a TX ring buffer before any printf().
     * Without this, printf() falls back to a busy-wait, character-by-character
     * poll of the UART with no task yield — at 115200 baud a large CSV row (many
     * sensors enabled for printing) can spin the CPU long enough to starve the
     * IDLE task and trip the watchdog. Installing the driver makes writes
     * interrupt-driven so the printing task actually blocks/yields instead. */
