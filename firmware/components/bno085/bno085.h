 ...
 *
 * Pure math utility — does not require a handle or hardware, and works on any quaternion
 * from this API's data union (.quaternion, .game_rotation_vector, .geomagnetic_rotation_vector,
 * .ar_vr_stabilized_rv, .gyro_integrated_rv all share the same {i, j, k, real} layout).
 *
 * @param i, j, k, real Quaternion components.
 * @param out_roll Pointer to receive roll in radians (NULL to skip).
 * @param out_pitch Pointer to receive pitch in radians (NULL to skip).
 * @param out_yaw Pointer to receive yaw in radians (NULL to skip).
 */
